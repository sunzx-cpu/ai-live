...
<script>
  import AddOrUpdate from './live-interact-keyword-add-or-update'
  export default {
    data () {
      return {
        dataForm: {
    ...
    methods: {
      ...
      getDataList () {
        ...
        this.$http.get('/apps/live-interact-keyword/list', {
          'page': this.pageIndex,
          'limit': this.pageSize,
          ...this.dataForm
        }).then(({data}) => {
      ...
      deleteHandle (id) {
        ...
        this.$http.delete('/apps/live-interact-keyword/delete', ids).then(({data}) => {
            if (data && data.code === 0) {
              this.$message({
                message: '操作成功',
                type: 'success',
      ...
      exportExcelHandle () {
        if (this.dataListSelections.length <= 0) {
          this.$message.warning('请选择要导出的关键词')
          return
        }

        this.$confirm('确定要导出选中的关键词到Excel吗？', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'info'
        }).then(() => {
          ...
          // 使用HTTP请求下载Excel文件
          this.$http({
            url: this.$http.adornUrl('/apps/live-interact-keyword/exportExcel'),
            method: 'get',
            ...
            link.href = url
            link.download = `live-interact-keywords-${new Date().getTime()}.xls`
      ...
      handleExcelFileImport (event) {
        ...
        this.$confirm('确定要导入Excel关键词吗？导入后将创建新的关键词记录。', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'info'
        }).then(() => {
          this.$http({
            url: this.$http.adornUrl('/apps/live-interact-keyword/importExcel'),
            method: 'post',
            data: formData,
            headers: {
