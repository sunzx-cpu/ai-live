  ...
  <div class="mod-config">
    <el-form :inline="true" :model="dataForm" @keyup.enter.native="search()">
      <el-form-item prop="scriptId">
        <el-select v-model="dataForm.scriptId" filterable placeholder="选择脚本">
          <el-option v-for="item in scriptList" :key="item.id" :label="item.name" :value="item.id"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item prop="actionType">
        <el-select v-model="dataForm.actionType" placeholder="行为名称">
          <el-option v-for="item in actionTypeList" :key="item" :label="item" :value="item"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item>
...
<script>
  import AddOrUpdate from './live-interact-action-add-or-update'
  export default {
    data () {
      return {
        dataForm: {
        ...
        actionTypeList: ['粉丝团消息', '礼物消息', '关注消息', '进入直播间', '点赞消息'],
        scriptList: []
      }
    },
    components: {
    ...
    methods: {
      ...
      getDataList () {
        ...
        this.$http.get('/apps/live-interact-action/list', {
          'page': this.pageIndex,
          'limit': this.pageSize,
          ...this.dataForm
        }).then(({data}) => {
      ...
      deleteHandle (id) {
        ...
        this.$http.delete('/apps/live-interact-action/delete', ids).then(({data}) => {
            if (data && data.code === 0) {
              this.$message({
                message: '操作成功',
                type: 'success',
      ...
      exportExcelHandle () {
        if (this.dataListSelections.length <= 0) {
          this.$message.warning('请选择要导出的行为互动')
          return
        }

        this.$confirm('确定要导出选中的行为互动到Excel吗？', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'info'
        }).then(() => {
          ...
          // 使用HTTP请求下载Excel文件
          this.$http({
            url: this.$http.adornUrl('/apps/live-interact-action/exportExcel'),
            method: 'get',
            ...
            link.href = url
            link.download = `live-interact-actions-${new Date().getTime()}.xls`
      ...
      handleExcelFileImport (event) {
        ...
        this.$confirm('确定要导入Excel行为互动吗？导入后将创建新的行为互动记录。', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'info'
        }).then(() => {
          this.$http({
            url: this.$http.adornUrl('/apps/live-interact-action/importExcel'),
            method: 'post',
            data: formData,
            headers: {
