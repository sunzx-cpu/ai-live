...
let mainConfig = {
  ...
  plugins: [
    new webpack.DefinePlugin({
      ...
      // 🎯 注入开发构建标志
      'process.env.DEV_BUILD': JSON.stringify(process.env.DEV_BUILD || 'false')
    })
  ],
  resolve: {
    alias: {
 ...
 */
if (process.env.NODE_ENV === 'production') {
  // 🎯 开发构建版本不删除 console.log，方便调试
  const isDevBuild = process.env.DEV_BUILD === 'true'

  if (config.build.cleanConsole && !isDevBuild) {
    // 生产版本：删除 console.log
